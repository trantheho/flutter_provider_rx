  ...
  BaseError checkError<T>(dynamic error) {
      if (error is DioError) {
        ...
        if (error.type == DioErrorType.RESPONSE) {
          final errorResponse = ErrorResponse(error.response.data);
          return BaseError(code: errorResponse.errors.first.code, message: errorResponse.errors.first.message);
        }
        if (error.error is SocketException) if (error.error.osError != null) {
          if (error.error.osError?.errorCode == 8 || error.error.osError?.errorCode == 7) {
            return BaseError(code: error.error.osError?.errorCode ?? -999, message: "network offline");
          } else {
            return BaseError(code: error.error.osError?.errorCode ?? -999, message: error.error.osError.message);
          }
        } else {
          if(!MainAppBloc.instance.showAlertTimeOut){
            AppHelper.showAppDialog(
                "title",
                "message");
          }
          return BaseError(code: error.error.osError?.errorCode ?? -999, message: 'Network error, please try again');
        }
      }
      ...
      if(error is HttpResponse){
        ...
        return BaseError(code: error.statusCode, message: 'An error occurred, Please try again later');
      }
